---
title: 配置
description: 本页面涵盖 Aether 靈境 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。
---
...
本页面涵盖 Aether 靈境 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。

## config.yaml

`config.yaml` 是 Aether 靈境 的主要配置文件。所有 Agent 行为、模型选择、工具加载和运行时功能都由它控制。
...
技能启用状态会反映在 `extensions_config.json` 中。你可以直接编辑它，或通过 Aether 靈境 应用界面进行管理。

## 运行时环境变量

这些变量在 Aether 靈境 进程中设置（通过 `.env`、Docker 环境或 shell）：
