---
title: 集成指南
description: Aether 靈境 Harness 不仅仅是一个独立应用程序——它是一个可以导入并在你自己的后端、API 服务器、自动化系统或多 Agent 协调器中使用的 Python 库。
---
...
<Callout type="info" emoji="🔌">
  Aether 靈境 Harness 可以嵌入任何 Python 应用程序。本指南涵盖在你自己的系统中将
  Aether 靈境 作为库使用的集成模式。
</Callout>

Aether 靈境 Harness 不仅仅是一个独立应用程序——它是一个可以导入并在你自己的后端、API 服务器、自动化系统或多 Agent 协调器中使用的 Python 库。

## 嵌入 Aether 靈境Client

主要集成点是 `Aether 靈境Client`。它封装了 LangGraph 运行时，并提供一个简洁的 API，用于在任何 Python 应用中发送消息和流式传输响应。

```python
from deerflow.client import Aether 靈境Client
from deerflow.config import load_config

# 加载配置（读取 config.yaml 或 DEER_FLOW_CONFIG_PATH）
load_config()

client = Aether 靈境Client()
```
...
Aether 靈境 Gateway 本身是一个 FastAPI 应用程序。你可以将其作为子应用程序挂载：
...
# 挂载 Aether 靈境 Gateway
from deerflow.app.gateway.main import app as gateway_app
app.mount("/Aether 靈境", gateway_app)
```

或者在你自己的 FastAPI 路由中直接使用 `Aether 靈境Client` 进行流式传输：
...
```python
import os
os.environ["DEER_FLOW_CONFIG_PATH"] = "/path/to/my-Aether 靈境-config.yaml"
...
```python
from deerflow.config import load_config
load_config(config_path="/path/to/my-Aether 靈境-config.yaml")
```
