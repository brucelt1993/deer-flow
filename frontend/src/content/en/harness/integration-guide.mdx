...
description: Aether Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
---
...
<Callout type="info" emoji="🔌">
  Aether Harness can be embedded into any Python application. This guide
  covers the integration patterns for using Aether as a library inside your
  own system.
</Callout>

Aether Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.

## Embedding AetherClient

The primary integration point is `AetherClient`. It wraps the LangGraph runtime and exposes a clean API for sending messages and streaming responses from any Python application.

```python
from deerflow.client import AetherClient
...
Aether Gateway is itself a FastAPI application. You can mount it as a sub-application or router:
...
# Mount the Aether gateway router
from deerflow.app.gateway.main import app as gateway_app
app.mount("/Aether", gateway_app)
```

Or use `AetherClient` directly in your own FastAPI routes with streaming:
...
Aether Harness is built on LangGraph. The Lead Agent is a standard LangGraph graph. You can compose it with your own LangGraph nodes and graphs:
...
```python
import os
os.environ["DEER_FLOW_CONFIG_PATH"] = "/path/to/my-Aether-config.yaml"
...
```python
from deerflow.config import load_config
load_config(config_path="/path/to/my-Aether-config.yaml")
```

## MCP server integration

Aether can expose its agent as an MCP server, allowing other MCP-compatible systems to call it as a tool. Refer to the Aether repository for MCP server integration examples.
