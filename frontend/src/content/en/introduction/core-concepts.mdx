---
title: Core Concepts
description: Before you go deeper into Aether, it helps to anchor on a few concepts that appear throughout the system. These concepts explain what Aether is optimizing for and why its architecture looks the way it does.
---
...
<Callout type="important" emoji="🧠">
  Aether makes the most sense if you think of it as a runtime for long-horizon
  agents, not just a chat interface or a workflow graph.
</Callout>

Before you go deeper into Aether, it helps to anchor on a few concepts that appear throughout the system. These concepts explain what Aether is optimizing for and why its architecture looks the way it does.

## Harness

In Aether, a **harness** is the runtime layer that gives an agent the environment it needs to do real work.
...
In practice, Aether's harness includes things like:
...
That is why Aether is not only a model wrapper and not only a workflow graph. It is a runtime environment for agents.
...
Aether is designed for this kind of work. Its architecture assumes that useful tasks often take more than one tool call and more than one reasoning pass.
...
In Aether, deep research is one skill. Data analysis, content generation, design-oriented workflows, and other task families can also be represented as skills.

This is a major part of the Aether mental model: the runtime stays general, while skills provide specialization.

## Sandbox

A **sandbox** is the isolated execution environment where the agent does file and command-based work.

Instead of treating the agent as a pure text generator, Aether gives it a workspace where it can read files, write outputs, run commands, and produce artifacts. This makes the system much more useful for coding, analysis, and multi-step workflows.

Isolation matters because execution should be controlled and reproducible. The sandbox is what lets Aether support action, not just conversation.

## Subagent

A **subagent** is a focused worker that handles a delegated subtask.

When a task is too broad for one reasoning thread, Aether can split the work into smaller units and run them separately. Subagents help with parallel exploration, scoped execution, and reducing overload on the main agent.
...
Long tasks put pressure on the context window. If everything is kept inline forever, the agent becomes slower, noisier, and less reliable. Aether addresses this with techniques such as summarization, scoped context for subagents, and using the file system as external working memory.
...
**Memory** is Aether's mechanism for carrying useful information across sessions.
...
In Aether, memory is part of the runtime, not an afterthought layered on top.
...
This matters because Aether is designed around task completion, not just answer generation. The system is trying to produce something you can inspect, refine, or hand off.
...
If you keep that model in mind, the rest of the Aether docs will be much easier to navigate.

<Cards num={2}>
  <Cards.Card title="Why Aether" href="/docs/introduction/why-aether" />
